...
func Test1(t *testing.T) {
	fmt.Println(OptionMethodInvocation)
	fmt.Println(OptionSubscriptions)
	server := NewServer()
	_ = server
}
...
func TestServerRegisterName(t *testing.T) {
	...
	svc, ok = server.services.services["rpc"]
	for name, cb := range svc.callbacks {
		fmt.Println(name)
		_ = cb
	}
	for name, cb := range svc.subscriptions {
		fmt.Println(name + "***")
		_ = cb
	}
	svc, ok = server.services.services["test"]
...
//测试脚本，在clientConn中一写N收
func runTestScript(t *testing.T, file string) {
	if file != "testdata\\subscription.js" {
		return
	}
	logs.Info("======================" + file + "======================")
...
func TestServerShortLivedConn(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		if !bytes.Equal(buf[:n], []byte(wantResp)) {
			...
			logs.Error("wrong response: %s", buf[:n])
		}
		str := string(buf)
