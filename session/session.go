...
//GetSid ..
func (manager *Manager) GetSid(r *http.Request) (string, error) {
	return manager.getSid(r)
}

// SessionStart generate or read the session id from http request.
// if session id exists, return SessionStore with this id.
//2019.04.18  forceSessionid强行指定sessionid，不从cookie中读取
func (manager *Manager) SessionStart(w http.ResponseWriter, r *http.Request, forceSessionid string) (session Store, err error) {
	sid := forceSessionid
	var errs error

	if sid == "" { //没有指定时，尝试从cookie获取
	...
	// Generate a new session  //没有sid时才创建
	if sid == "" {
